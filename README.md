...
- **Workflow Runtime (v1):** a minimal deterministic, durable planner/executor runtime that executes a validated spec over the durable control store + Agent Gateway, surviving runtime/Gateway restarts without duplicating Agent Tasks: see [`docs/workflow-runtime.md`](docs/workflow-runtime.md).
- **AgentTaskResult:** a first-class, durable, authoritative task result (final output + status: available/missing/invalid) persisted on the Node (encrypted `run_result_v1`) and Gateway, so the Workflow Runtime routes on the result instead of reconstructing control output from event history: see [`docs/agent-task-result.md`](docs/agent-task-result.md).
- **Workflow API + MCP:** the Workflow Runtime lifecycle (list/create/start/get/events/cancel) exposed through the same `vibe api serve` REST server (`/v1/workflows`) and `vibe mcp serve` MCP tools — create is separate from explicit start; `blocked` is non-terminal; a wait timeout never cancels: see [`docs/workflow-api.md`](docs/workflow-api.md).
